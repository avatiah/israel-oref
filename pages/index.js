...
// Выносим Gauge в чистый компонент без внешних зависимостей
...
export default function Home() {
  ...
  useEffect(() => {
    const load = () => fetch('/api/data').then(r => r.json()).then(d => setData(d)).catch(e => console.log(e));
    load();
    const int = setInterval(load, 30000);
    return () => clearInterval(int);
  }, []);

  if (!data) return <div style={{background:'#000', color:'#0f0', height:'100vh', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'monospace'}}>STABILIZING_V41...</div>;
  ...
  return (
    <div className="dashboard">
      ...
      {/* ДИНАМИЧЕСКАЯ ЛЕНТА БЕЗ API */}
      <section className="card log-card">
        <div className="section-title green">RAW_SIGNAL_STREAM // OSINT_LIVE</div>
        <div className="feed-box">
           {(data.feed || data.logs).map((l, i) => (
             <div key={i} className="log-entry white">
               <span style={{color: '#0f0', marginRight: '8px'}}>[{new Date().toLocaleTimeString()}]</span> {l}
             </div>
           ))}
        </div>
      </section>
      ...
      <style jsx global>{`
        ...
        .feed-box { height: 150px; overflow-y: auto; }
        .log-entry { font-size: 0.65rem; padding: 5px 0; border-bottom: 1px solid #111; line-height: 1.3; }
