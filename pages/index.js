import React, { useEffect, useState, useRef } from 'react';

// Компонент спидометра (SVG Gauge)
const Gauge = ({ value, label, color }) => {
  const radius = 40;
  const stroke = 8;
  const normalizedValue = Math.min(Math.max(value, 0), 100);
  const circumference = Math.PI * radius; // Полукруг
  const strokeDashoffset = circumference - (normalizedValue / 100) * circumference;

  return (
    <div style={s.gaugeContainer}>
      <svg width="140" height="80" viewBox="0 0 100 60">
        {/* Фоновая дуга */}
        <path
          d="M 10 50 A 40 40 0 0 1 90 50"
          fill="none"
          stroke="#111"
          strokeWidth={stroke}
          strokeLinecap="round"
        />
        {/* Активная дуга (Прогресс) */}
        <path
          d="M 10 50 A 40 40 0 0 1 90 50"
          fill="none"
          stroke={color}
          strokeWidth={stroke}
          strokeDasharray={circumference}
          strokeDashoffset={strokeDashoffset}
          strokeLinecap="round"
          style={{ transition: 'stroke-dashoffset 0.8s ease-out, stroke 0.5s ease' }}
        />
        <text x="50" y="45" textAnchor="middle" fill="#fff" fontSize="14" fontWeight="bold" fontFamily="monospace">
          {value}%
        </text>
        <text x="50" y="58" textAnchor="middle" fill={color} fontSize="6" fontWeight="bold" fontFamily="monospace">
          {label}
        </text>
      </svg>
    </div>
  );
};

export default function MadadHaOref() {
  const [data, setData] = useState(null);
  const lastValidData = useRef(null);

  const fetchIntel = async () => {
    try {
      const res = await fetch('/api/data');
      if (!res.ok) throw new Error();
      const json = await res.json();
      if (json?.nodes) {
        lastValidData.current = json;
        setData(json);
      }
    } catch (e) {
      if (lastValidData.current) setData(lastValidData.current);
    }
  };

  useEffect(() => {
    fetchIntel();
    const timer = setInterval(fetchIntel, 10000);
    return () => clearInterval(timer);
  }, []);

  if (!data) return <div style={s.loader}>{">"} ACCESSING_INTEL_STREAM...</div>;

  return (
    <div style={s.container}>
      <header style={s.header}>
        <h1 style={s.logo}>MADAD HAOREF</h1>
        <div style={s.statusBlock}>
          <div style={s.meta}>SECURITY_TERMINAL_V12.0</div>
          <div style={s.statusText}>STATUS: <span style={{color: '#0f4'}}>LIVE_ENCRYPTED</span></div>
          <div style={s.time}>{new Date(data.timestamp).toLocaleTimeString()} UTC</div>
        </div>
      </header>

      <main style={s.grid}>
        {data.nodes.map(node => (
          <div key={node.id} style={s.card}>
            <div style={s.cardLayout}>
              {/* Левая часть: Графика */}
              <Gauge 
                value={node.value} 
                label={node.id} 
                color={node.value > 65 ? '#ff3e3e' : '#0f4'} 
              />
              
              {/* Правая часть: Заголовок и данные */}
              <div style={s.cardContent}>
                <div style={s.nodeTitle}>{node.title}</div>
                <div style={s.newsSection}>
                  {node.news?.map((item, idx) => (
                    <div key={idx} style={s.newsItem}>
                      <span style={s.newsSrc}>[{item.src}]</span> {item.txt}
                    </div>
                  ))}
                </div>
              </div>
            </div>
            
            <div style={s.infoBox}>
              <div style={s.infoRow}>
                <span style={s.infoLabel}>METRICS:</span> 
                <span style={s.infoVal}>NOTAMs, DIPLOMACY, ENERGY_PRICES, USD/ILS.</span>
              </div>
            </div>
          </div>
        ))}

        <div style={s.forecastBox}>
          <h3 style={s.forecastTitle}>⚠️ СЦЕНАРНЫЙ ПРОГНОЗ (06.02)</h3>
          <p style={s.forecastText}>
            ТЕКУЩИЙ ВЕКТОР: <strong style={{color:'#ff3e3e'}}>{data.prediction?.status}</strong>. <br/>
            ВЕРОЯТНОСТЬ КИНЕТИЧЕСКОГО УДАРА США: <strong>{data.prediction?.impact}%</strong>.
          </p>
        </div>
      </main>

      <footer style={s.footer}>
        <div style={s.footerMeta}>MADAD HAOREF © 2026 // ADAPTIVE_OSINT_ENGINE</div>
      </footer>
    </div>
  );
}

const s = {
  container: { background: '#000', color: '#0f4', fontFamily: 'monospace', minHeight: '100vh', padding: '40px 15px', display: 'flex', flexDirection: 'column', alignItems: 'center' },
  header: { textAlign: 'center', marginBottom: '40px', width: '100%', maxWidth: '650px', borderBottom: '1px solid #222', paddingBottom: '20px' },
  logo: { fontSize: 'clamp(24px, 8vw, 36px)', letterSpacing: '5px', fontWeight: 'bold', margin: '0' },
  statusBlock: { marginTop: '10px' },
  meta: { fontSize: '10px', color: '#00cc00' },
  statusText: { fontSize: '11px', fontWeight: 'bold', margin: '5px 0' },
  time: { fontSize: '10px', color: '#008800' },
  
  grid: { width: '100%', maxWidth: '650px', display: 'flex', flexDirection: 'column', gap: '20px' },
  card: { border: '1px solid #004400', padding: '20px', background: '#050505' },
  cardLayout: { display: 'flex', alignItems: 'center', gap: '20px', flexWrap: 'wrap', justifyContent: 'center' },
  gaugeContainer: { flexShrink: 0 },
  cardContent: { flex: '1 1 300px' },
  
  nodeTitle: { fontSize: '13px', color: '#fff', fontWeight: 'bold', marginBottom: '15px', borderBottom: '1px solid #111', paddingBottom: '5px' },
  newsSection: { display: 'flex', flexDirection: 'column', gap: '8px' },
  newsItem: { fontSize: '12px', color: '#eee', textTransform: 'none', lineHeight: '1.4' },
  newsSrc: { color: '#0f4', fontWeight: 'bold' },
  
  infoBox: { borderTop: '1px solid #1a1a1a', paddingTop: '15px', marginTop: '15px' },
  infoRow: { display: 'flex', fontSize: '9px' },
  infoLabel: { color: '#0f4', width: '70px' },
  infoVal: { color: '#ffffff', textTransform: 'none' },

  forecastBox: { border: '1px solid #600', padding: '20px', background: '#100000', textAlign: 'center' },
  forecastTitle: { fontSize: '14px', color: '#ff3e3e', margin: '0 0 10px 0' },
  forecastText: { fontSize: '13px', textTransform: 'none', color: '#fff' },
  
  footer: { marginTop: '50px', borderTop: '1px solid #1a1a1a', paddingTop: '20px', width: '100%', maxWidth: '650px', textAlign: 'center' },
  footerMeta: { fontSize: '9px', color: '#004400' },
  loader: { height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#000', color: '#0f4' }
};
